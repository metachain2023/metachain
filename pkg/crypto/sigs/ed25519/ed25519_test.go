...
func TestRoundtrip(t *testing.T) {
	priv, err := sigs.Generate(crypto.TypeED25519)
	require.NoError(t, err)

	msg := []byte("kortho")
	sig, err := sigs.Sign(crypto.TypeED25519, priv, msg)
	require.NoError(t, err)

	pk, err := sigs.ToPublic(crypto.TypeED25519, priv)
	require.NoError(t, err)
	t.Log(len(pk))
	t.Log("protocl:", pk[0])
