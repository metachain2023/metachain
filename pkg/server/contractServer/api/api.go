	...
	case ETH_GETBALANCE:
		...
		if err != nil || len(para) == 0 {
			...
			if err != nil {
				...
				bigB := new(big.Int).Set(blc)

				resBalance := fmt.Sprintf("%X", bigB)
...
func (s *Server) eth_getBalance(from string) (*big.Int, error) {
...
func (s *Server) eth_getBlockByHash(hash string, bl bool) (*Block, error) {
	...
	return s.blockToEthBlock(b, bl), nil
}
...
func (s *Server) eth_getTransactionByHash(hash string) (*Transaction, error) {
	...
	var trs Transaction
	// from, _ := s.cli.AddressToCommonAddr(tx.From)
	trs.From = *tx.From
	// to, _ := s.cli.AddressToCommonAddr(tx.To)
	trs.To = *tx.To
	...
	trs.GasPrice = tx.GasPrice.String()
	trs.Gas = tx.GasUsed.String()
	trs.Hash = common.BytesToHash(tx.Hash())
	trs.Nonce = uint64ToHexString(tx.Nonce)
	trs.Value = tx.Amount.String()
...
func (s *Server) eth_getTransactionReceipt(hash string) (*TransactionReceipt, error) {
	...
	return s.txToTxReceipt(tx, b), nil
}
...
// block to eth block
func (s *Server) blockToEthBlock(b *block.Block, bl bool) *Block {
	var block Block
	block.Difficulty = stringToHex(b.GlobalDifficulty.Text(16))
	// block.GasUsed = uint64ToHexString(b.GasUsed)
	block.GasUsed = b.GasUsed.String()
	...
	// miner, _ := s.cli.AddressToCommonAddr(b.Miner)
	block.Miner = *b.Miner

	if bl {
		block.Transactions = s.txsToEthTxs(b, b.Transactions)
	} else {
...
// txs to eth txs
func (s *Server) txsToEthTxs(block *block.Block, ktxs []*transaction.FinishedTransaction) []*TransactionReceipt {
	var etxs []*TransactionReceipt
	for _, ktx := range ktxs {
		etxs = append(etxs, s.txToTxReceipt(ktx, block))
	}
	return etxs
}
...
// tx to eth tx receipt
func (s *Server) txToTxReceipt(tx *transaction.FinishedTransaction, block *block.Block) *TransactionReceipt {
