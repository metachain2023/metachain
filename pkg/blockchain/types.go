...
var (
	coinsPerYear  = 365 * 24 * 60
	initialReward = big.NewInt(8)
)

func GetMinerAmount(height uint64) *big.Int {

	reward := new(big.Int).Mul(initialReward, transaction.UnitPrecision)
	years := height / uint64(coinsPerYear)

	for i := 0; i < int(years); i++ {
		reward.Mul(reward, big.NewInt(80))
