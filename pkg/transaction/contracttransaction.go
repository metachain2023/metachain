...
func VerifymeteSign(kFrom *common.Address, msgHash []byte, ethdata string) bool {
	...
	if len(sign) <= 64 {
		logger.Error("Verify Sign lenght error", zap.Int("length:", len(sign)))
		return false
	}
...
func DecodeTxData(data []byte) (*FinishedTransaction, error) {
	var ftx FinishedTransaction
	err := json.Unmarshal(data, &ftx)
	if err != nil {
		logger.Error("gDecodeTxData error", zap.Error(err))
		return nil, err
	}
	return &ftx, nil
}
